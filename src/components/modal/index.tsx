...
export default function ModalMine(props: ModalTypes) {
  ...
  return (
    <>
      <Modal
        className={`p-2  md:1/3 lg:w-1/4 w-full ${props.modalProps?.useDatePicker === true && '  lg:h-1/4  h-2/5 w-full'} ${props.modalProps?.useDropdownChangeParents === true && 'lg:h-[20vh] '} ${props.modalProps?.useDesativar === true && 'lg:h-[20vh] lg:w-[20vw] '} ${props.modalProps?.useTaxForTransaction === true && 'h-[80vh] lg:h-[40vh]'} max-w-[90vw] `}
        isOpen={props.isOpen}
            ...
            <>
              <ModalHeader className="flex flex-col items-center justify-center gap-1">
                {props.modalProps?.useDatePicker === true
                  ...
                  : props.modalProps?.action}
              </ModalHeader>
              <ModalBody
                className={`flex flex-col items-center ${props.modalProps?.useDatePicker === true ? 'justify-center' : 'justify-end'}`}
              >
                {props.modalProps?.useDatePicker === true && (
                ...
                {props.modalProps?.useDropdownChangeParents === true && (
                  <div className="w-full max-w-full">
                    <Dropdown>
                      <DropdownTrigger>
                        <Button variant="flat" color="primary" fullWidth={true}>
                          {selected || 'Selecione um markeplace'}
                          <CaretDown size={18} />
                        </Button>
                      </DropdownTrigger>
                      ...
                      >
                        {props.MarketplacesArray ? (
                          props.MarketplacesArray.map(
                            (marketplace: objectMarketplace) => (
                              <DropdownItem
                                key={marketplace.id}
                                className="max-w-[70vw]"
                                onClick={() =>
                                  setSelected(marketplace.nome_fantasia)
                                }
                              >
                  ...
                  <div className=" h-full w-full flex flex-col items-center p-4 gap-2">
                    ...
                    <div className="w-full flex flex-col items-center justify-center  h-full">
                      ...
                      <Input
                        ...
                        onChange={props.onChangeTaxTransaction}
                        name="amount"
                        type="number"
                        variant="underlined"
                        label="Valor R$"
                ...
                {props.modalProps?.useDesativar === true && (
                  <div className=" text-lg h-full flex items-center">
                    Desativar Marketplace?
                  </div>
                )}
              </ModalBody>
